...
module.exports = function(config) {
  ...
  return {
    ...
    records: {
      ...
      get: auto(function(record, options, callback) {
        adapter.records.get(record, callback);
      }),
      add: auto(function(domain, record, options, callback) {
        adapter.records.add(domain, record, callback);
      }),
      update: auto(function(domain, record, options, callback) {
        adapter.records.update(domain, record, callback);
      }),
      remove: auto(function(domain, record, options, callback) {
        adapter.records.remove(domain, record, callback);
      }),
      removeById: auto(function(domain, record, options, callback) {
        adapter.records.removeById(domain, record, callback);
      })
    }
  }
}
