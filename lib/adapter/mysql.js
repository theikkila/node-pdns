...
var adapter = module.exports = {
  ...
  domains: {
    ...
    remove: function(domain, callback) {
      ...
      client.query('DELETE domains, records FROM domains '
                  +'LEFT JOIN records '
                  +'ON records.domain_id = domains.id '
                  +'WHERE domains.name LIKE ? AND domains.type LIKE ?'
                  +' AND domains.ttl LIKE ? AND domains.created_at LIKE ?'
                  +' AND domains.updated_at LIKE ?',
       [domain.name, domain.type || '%',
       domain.ttl || '%', domain.created_at || '%', 
       domain.updated_at || '%'], callback);
    }
  },
  ...
  records: {
    ...
    get: function(record, callback) {
      ...
      record = record || {};
      client.query('SELECT * FROM records WHERE id=?',
       [record.id],
       function(err, results, fields) {
        if (err) {
          callback(err);
    ...
    update: function(domainName, record, callback) {
      if(!checkDomainName(domainName)) {
        callback(new Error("domain invalid"));
        return;
      }
      ...
      client.query('UPDATE records SET '
        +'name=?, content=?, ttl=?, '
        +'change_date=UNIX_TIMESTAMP(), updated_at=NOW()'
        +'WHERE id=?',
        [record.name, record.content, record.ttl || 84600, record.id],
        function(err, results) {
          if(err) {
            callback(err);
          } else {
    ...
    removeById: function(domainName, record, callback) {
      if(!checkDomainName(domainName)) {
        callback(new Error("domain invalid"));
        return;
      }
      client.query('DELETE FROM records WHERE id=?',
       [record.id],
       function(err, results) {
        if(err || !results) {
          callback(err || new Error("no match for query"));
